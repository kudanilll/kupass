...
| Unit: ViewModels | Yes (4) | Editor create/update (encrypted at rest), Home list + search, Detail delete | `ui/screens/ViewModelsTest.kt` with `testing/FakePasswordDao` + `MainDispatcherRule` (kotlinx-coroutines-test) |
| Unit: app lock | Yes (9) | Initial lock, timeout boundaries, immediate, rotation, file-picker exemption (single use, expiring), locked departures | `security/AppLockTest.kt` (fake clock) |
...
| Unit: repository | Yes (5) | Full-field encryption at rest, case-insensitive sort, in-memory search on all fields, legacy upgrade + idempotence, undecryptable rows untouched | `data/repository/PasswordRepositoryTest.kt`. Device: `androidTest/.../PasswordRepositoryUpgradeTest` (real Keystore + Room) |
