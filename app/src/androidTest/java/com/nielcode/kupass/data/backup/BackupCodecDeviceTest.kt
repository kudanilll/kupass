...
class BackupCodecDeviceTest {
    ...
    fun productionIterationsRoundTripWithinBudget() {
        ...
        Log.i(
            "BackupCodecDeviceTest",
            "PBKDF2 ${BackupCodec.DEFAULT_ITERATIONS} iterations: encode=${encodeMs}ms, encode+decode=${totalMs}ms",
        )

        assertEquals(entries.map { it.copy(id = 0) }, restored)
        // Generous budget for slow emulators; a real regression (e.g. 10x iterations) would blow
        // it.
