...
fun VaultEvent.message(context: Context): String =
    when (this) {
        ...
        is VaultEvent.ImportSucceeded ->
            if (skipped == 0) {
                context.resources.getQuantityString(R.plurals.toast_import_result, imported, imported)
            } else {
                context.resources.getQuantityString(R.plurals.toast_import_result, imported, imported) + " " +
                    context.resources.getQuantityString(R.plurals.toast_import_skipped, skipped, skipped)
            }
